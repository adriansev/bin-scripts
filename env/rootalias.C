...
static TObject* Apply2Ptr (TObject* obj, const char* arg) {
  std::stringstream obj_str (""), cname_str ("");
  TString class_name ("");
  TString* gint_class_id = NULL;

  obj_str << obj;
  cname_str << &class_name ;
...
static void ForEachH (TString arg = "") {
  ...
  for (int i=0; i<list_canvases->GetEntries(); ++i) {
    ...
    for (int i=0; i<glist->GetEntries(); ++i) {
      ...
      if ( obj->InheritsFrom("TH1") ) { Apply2Ptr (obj, arg.Data() ); canvas->Modified(); }
      ...
      if ( obj->InheritsFrom("TPad") ) {
        ...
        for (int i=0; i<padlist->GetEntries(); ++i) {
          ...
          if ( h_in_padlist->InheritsFrom("TH1") ) { Apply2Ptr (h_in_padlist, arg.Data() ); pad->Modified(); }
          }
        }
    }
  }
